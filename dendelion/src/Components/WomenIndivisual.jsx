...
let WomenIndivisual = () => {
  ...
  useEffect(() => {
    fetch("https://dendelion-54b09-default-rtdb.firebaseio.com/women.json")
      .then((res) => res.json())
      .then((data) => {
        let WomenIndividualData = Object.values(data).filter((item) => {
          return item.name === WomenName;
        });
        setSingleData(WomenIndividualData[0]);
        console.log(WomenIndividualData[0])
      });
  }, []);
...
export default WomenIndivisual;